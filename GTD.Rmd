...
One could conclude that those `r DeathsGFA[2]` deaths caused by global terrorists in the years since the GFA are almost entirely accounted for in Madrid and London bombings and the Paris attacks. Clearly these are very deadly incidents.
...
The modern extremist threat is a lot more deadly per incident, but also a good deal more rare. This is something else that can easily be visualised:
...
```{r}
gtn.glm2 <- glm(ninc+1~yr, data = nincs
                , family = Gamma(link=log))
anova(gtn.glm2, test = "Chisq")
plot(ninc~yr, data = nincs)
points(predict(gtn.glm2, type = "response")~yr, data = nincs, col = "red")
```

```{r}
gtn.glm3 <- glm(ninc+1~yr, data = nincs
                , family = poisson)
anova(gtn.glm3, test = "Chisq")
plot(ninc~yr, data = nincs)
points(predict(gtn.glm3, type = "response")~yr, data = nincs, col = "red")
```

```{r}
gtn.glm4 <- glm(ninc+1~poly(yr,4), data = nincs
                , family = Gamma(link=log))
anova(gtn.glm4, test = "Chisq")
plot(ninc~yr, data = nincs)
points(predict(gtn.glm4, type = "response")~yr, data = nincs, col = "red")
```
...
```{r}
gtd.glm1 <- glm(fatalities~poly(yr,2) * International.Extremism
...
plot(fatalities ~ Year, data = gtdgroups, col = ieCols, pch = ".", ylim = c(0,150))
