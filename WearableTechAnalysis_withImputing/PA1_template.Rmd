...
```{r initial_attempt_to_calculate_means}
...
```{r histogram_occurance_of_NA_each_day}
...
```{r mean_and_max_steps_per_inteval}
...
```{r lineplot_mean_steps_per_interval}
...
```{r create_new_dataset_with_imputed}
...
```{r calculate_mean_and_med_adjusted}
...
```{r barplot_total_steps_per_day_adjusted}
...
```{r determine_weekday_or_weekend}
...
```{r lineplot_mean_steps_per_interval_by_weekpart}
