...
*From these mosaic plots, it can be seen that the best fit is the model of no three way interaction. However, a fit of conditional independance [Time Alive][Length Alive] is good enough and matches what is known about the experiment's design.
...
```{r}
cat("Trying mutual independace model first")
...
*This model seems to be poorer than the mutual independance model and hasn't returned a goodness of fit likelihood ratio. The shading in the mosaic suggests that there is still unexpectedly high incidence of disease category C2 and C1 with female parent 4 and an unexpected deficit in C1 with female parent 3.*
