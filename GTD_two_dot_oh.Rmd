...
```{r nfatNbinomGLMnozero}
gtf.glm0 <- glm(fatalities~nMonthsSinceJan1970, data = nfatm[nfatm$fatalities > 0,]
                , family = negative.binomial(theta = 1.27, link=log))

gtf.nlm0 <- glm(fatalities~bs(nMonthsSinceJan1970
                             , knots = c(60, 150, 240, 420))
               , data = nfatm[nfatm$fatalities > 0,]
               , family = negative.binomial(theta = 1.27, link=log))
anova(gtf.nlm, test = "Chisq")
```
...
```{r yfatNbinomGLMnozero}
gtf.glm20 <- glm(fatalities~nMonthsSinceJan1970, data = yfatm[yfatm$fatalities > 0,]
                , family = negative.binomial(theta = 0.07, link=log))

gtf.nlm20 <- glm(fatalities~ns(nMonthsSinceJan1970,3)
                , data = yfatm[yfatm$fatalities > 0,]
                , family = negative.binomial(theta = 0.07, link=log))
anova(gtf.nlm, test = "Chisq")
```
...
distplot(table(nincm$nIncidents), lambda = unlist(ninc.pois$par)[1]
        , xlab = "Number of incidents\nin a given month")
```
...
plot(ninc.nbin, shade = TRUE
    ...
    , xlab = "Number of incidents\nin a given month")
distplot(table(nincm$nIncidents), type = "nbinomial", size = unlist(ninc.nbin$par)[1]
        , xlab = "Number of incidents\nin a given month")
```
...
plot(yinc.pois, shade = TRUE
     , main = "poisson model"
     , xlab = "Number of incidents\nin a given month")
distplot(table(yincm$nIncidents), lambda = unlist(yinc.pois$par)[1]
        , xlab = "Number of incidents\nin a given month")
```
...
plot(yinc.nbin, shade = TRUE
    , main = "negative binomial model"
    , xlab = "Number of incidents\nin a given month")
distplot(table(yincm$nIncidents), type = "nbinomial", size = unlist(yinc.nbin$par)[1]
        , xlab = "Number of incidents\nin a given month")
```
...
Here we see a mostly flat trend in the glm with huge confidence intervals giving practically no useful information. The natural spline fit is dragged to the floor of the plot by the prevalence of zeros. One can see it lifts where it picks up the activity in the 1970's to 90's and again at the right extremity. Of course, Paris being the last point in the series has very high leverage and lifts the line rather dramatically. This is normal behaviour for this type of line fit, and is no indication of a dramatic escalation of risk.
...
In case it's not clear what's happening, here's the same plot ignoring all the months when there were zero incidents. You see the local average shoot up where there are just a very small number of rare events with a high death toll and stay low to the floor where there were a lot of incidents of mostly a single fatality.

```{r yfatNbinomGLMplotNoZero, fig.show='show', fig.height=4, fig.width=4}
gtf.glm.preds <- predict(gtf.glm20, newdata = yrmnth.grid, se = TRUE)
...
gtf.nlm.preds <- predict(gtf.nlm20, newdata = yrmnth.grid
                         , type = "terms"
                         , se = TRUE)
...
plot(log(fatalities)~nMonthsSinceJan1970
     ...
     , ylab = "log(Fatalities) in a given month"
     , main = "Fatalities from terrorist actions\n (international in Western Europe)"
     , sub = "Only months with > 0 incidents counted")
...
The huge confidence intervals in the peak regions show that the estimates are incredibly unreliable. There's not enough data and this highlights how rare the events have become. In the 1970's-90's the interval is compressed because the amount of data makes the estimates robust.
...
For the scatter plots, the challenge was to find a robust smoothing line that reveals trends in the count data where there is serious non-linearity and dramatic zero inflation.

I started with the assumption that number of fatalities in a given month follow standard models for count data, but initially struggled with it as can be seen from the first attempt to perform a poisson goodness of fit:
...
These are a lot better with much less evidence of zero inflation. However, there's still a real lack of fit above $e^2$. I want to find a model suitable for this situation.

A similar look at the number of incidents data:

```{r nincPoisFit, fig.show='show', fig.height=4, fig.width=4}
```

```{r nincNbinomFit, fig.show='show', fig.height=4, fig.width=4}
```

```{r yincPoisFit, fig.show='show', fig.height=4, fig.width=4}
```

```{r yincNbinomFit, fig.show='show', fig.height=4, fig.width=4}
```

Overall the negative binomial is the better fit for modeling the frequency of terrorist incidents by non-International extremists but needs to be corrected for a large zero inflation. Conversely it's a very good fit for the International group.
